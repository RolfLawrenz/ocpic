module SettingsHelper
  ...
  def setting_label_field(program, field, shooting_mode, time_mode)
    case field[:type]
      when :range
    ...
    label_tag("field_#{program}_#{shooting_mode}_#{time_mode}_#{field[:name]}", name, class: 'label_setting')
  end

  def setting_setting_field(program, field, shooting_mode, time_mode)
    html = ""
    case field[:type]
      when :toggle
        html =  "<div id='field_#{program}_#{shooting_mode}_#{time_mode}_#{field[:name]}' data-shooting-mode='#{shooting_mode}' data-time-mode='#{time_mode}' class=\"setting_setting_toggle toggle#{ field[:value] ? ' active' : ''}\">\n"
      ...
      when :date
        html = text_field_tag("field_#{program}_#{shooting_mode}_#{time_mode}_#{field[:name]}", field[:value], class: "setting_setting_date setting_field setting_text_field", data: {shooting_mode: shooting_mode, time_mode: time_mode})
    end
    html.html_safe
  end
